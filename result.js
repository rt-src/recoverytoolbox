...
convertPreviewTreeNodeToHtml = (previewTreeNode, topLevel = false) => {
    ...
    if (previewTreeNode.children instanceof Array && previewTreeNode.children.length > 0) {
        previewTreeNode.children.forEach((node) => {
                        ...
                        if (typeof previewJson.Tables !== 'undefined') {
                            ...
                            for (var tableKey in previewJson.Tables) {
                                if (Object.hasOwnProperty.call(previewJson.Tables, tableKey)) {
                                    ...
                                    leftPanelInnerHTML += `<a class="nav-link ${indx == 0 ? "active": ""}" id="v-pills-table-${indx}-tab" data-bs-toggle="pill" href="#table-${indx}" role="tab" aria-controls="v-pills-table-${indx}" aria-selected="true">${tableKey}</a>`
                                }
                            }
                                        ...
                                        table.forEach((row, position) => {
                                            ...
                                            if (row instanceof Array) {
                                                if (position == 0) {
                                                    ...
                                                    row.forEach((cell) => {
                                                        // if (pos == 0) {
                                                        //     rowString += `<th scope="row">${cell}</th>`;
                                                        // } else {
...
getResultInfoByOrderId = (orderId) => {
    if (debug) {
